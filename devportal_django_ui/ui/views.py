...
from .services import post_registration, post_login, post_logout, \
    ...
    get_swaggerprojects, get_swaggerlist, get_swagger_metrics, get_swagger_dependency, get_swagger_dependency_filter
...
def dependency(request):
    ...
    if request.method == 'GET':
        if 'getdepfilter' in request.GET:
            optionname = request.GET['depoption']
            if optionname == 'All':
                resp = get_swagger_dependency(to)
            else:
                resp = get_swagger_dependency_filter(to, optionname)
        else:
            resp = get_swagger_dependency(to)
    ...
    return render(request, 'ui/dependency.html', {'authenticated': au, 'admin': ad, 'd_s': d_s, 'options': options,
